...
let renderer, scene, camera, imageholder, eye, delta;
const sizes = {
...
function createFloor() {
    ...
    imageholder = new THREE.Mesh(new THREE.BoxGeometry(), new THREE.MeshBasicMaterial({
        color: 0xffffff,
        opacity: 0,
        transparent: true,
    }));
...
function initInstancing() {
    window.addEventListener("mousedown", (event) => {
        ...
        if (found.length > 0) {
            ...
            copyObject.position.set(found[0].point.x, found[0].point.y, found[0].point.z);
            copyObject.rotation.set(0, -pi / 2 + randFloat(-1, 1), 0);
            let endScale = randFloat(3, 8);
            copyObject.scale.set(endScale, endScale, endScale);
